...
# === 고정 경로: 시나리오2/kdh ===
BASE_DIR = r"C:\Users\권도혁\Desktop\시나리오1,2\시나리오2\kdh"
...
def summarize_enemy(enemy_team):
    ...
    for champ_name, _, role_name in enemy_team:
        ...
        if not row.empty:
            try: num_cc += int(row.iloc[0]["CCcount"])
            except: pass
...
if uploaded:
    temp_path = os.path.join(BASE_DIR, "temp", uploaded.name)
    ...
    try:
        ...
        with st.spinner("챔피언 10명 인식 중…"):
            champs10 = rc.extract_champions(temp_path)
        if champs10:
...
if go:  # 분석 시작 버튼 클릭 시
    with st.spinner("분석 중…"):
        try:
            rc = rc if "rc" in locals() else importlib.import_module("rune_champion")
            my_team, enemy_team = rc.extract_champions_and_runes(temp_path, my_champion)

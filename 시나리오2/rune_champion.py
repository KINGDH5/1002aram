...
# 고정 경로 (새 위치)
# ===============================
BASE_DIR = r"C:\Users\권도혁\Desktop\시나리오1,2\시나리오2"
VISION_KEY = os.path.join(BASE_DIR, "vision_key.json")
RUNE_KEY   = os.path.join(BASE_DIR, "gen-lang-client-0207006785-b6a30a13a4a1.json")

# ===============================
# 클라이언트
# ===============================
vision_creds = service_account.Credentials.from_service_account_file(VISION_KEY)
vision_client = vision.ImageAnnotatorClient(credentials=vision_creds)

RUNE_PROJECT_ID  = "gen-lang-client-0207006785"
RUNE_LOCATION    = "us-central1"
RUNE_ENDPOINT_ID = "3362037177390202880"
rune_creds = service_account.Credentials.from_service_account_file(RUNE_KEY)
...
champion_df = pd.read_csv(os.path.join(BASE_DIR, "lol_champions.csv"))
champions_list = champion_df["name"].tolist()
role_df = pd.read_csv(os.path.join(BASE_DIR, "champion_rune_roles.csv"))
...
def predict_RUNE(endpoint, image_bytes, threshold=35.0):
    instance = {"content": base64.b64encode(image_bytes).decode("utf-8")}
    try:
        res = endpoint.predict(instances=[instance])
        if not res.predictions: return "null", 0.0
        pred = res.predictions[0]
        if isinstance(pred, dict):
            names = pred.get("displayNames", []); confs = pred.get("confidences", [])
            if names and confs:
                i = confs.index(max(confs))
                best = RUNE_NAME_MAP.get(names[i], names[i]); conf = confs[i]*100
                return (best if conf >= threshold else "null", conf)
        return str(pred), 0.0
    except Exception as e:
        print(f"예측 오류: {e}"); return "null", 0.0

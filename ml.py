...
def train_models(df):
    ...
    y = df['win']
    X_train, X_test, y_train, y_test = train_test_split(X_onehot, y, test_size=0.2, random_state=SEED, stratify=y)
    synergy_model = xgb.XGBClassifier(objective='binary:logistic', eval_metric='logloss',
    ...
    # --- Champion-wise ---
    AURA_KEYS = ['damage_dealt','damage_taken','attack_speed','skill_haste','hp_regen',
                 'tenacity','shield_absorb','energy_regen']
    long_rows = []
    ...
    champ_feature_cols = [c for c in (AURA_KEYS + ["CCcount","champ1_is_ad_items","champ1_is_ap_items",
                                                  "champ1_is_tank_items","champ1_is_ranged"]) if c in champ_long.columns]
    Xc = champ_long[champ_feature_cols]
    yc = champ_long["win"]
    champ_model = xgb.XGBClassifier(n_estimators=150, max_depth=5, learning_rate=0.1, random_state=SEED, eval_metric='logloss')
    ...
    lvl_cols = [f"{s}{lvl}" for s in stat_types for lvl in lvl_suffixes if f"{s}{lvl}" in df.columns]
    ...
    y_stat = df['win']
    X_train_s, X_test_s, y_train_s, y_test_s = train_test_split(X_stat, y_stat, test_size=0.2, random_state=SEED, stratify=y_stat)

    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train_s)

    stat_model = xgb.XGBClassifier(n_estimators=200, max_depth=4, learning_rate=0.1, random_state=SEED, eval_metric='logloss')
...
def get_team_winrate(team_champs, models):
    ...
    for cand in team_champs:
        ...
        p = champ_model.predict_proba(r[[c for c in champ_profile.columns if c!="champion"]])[0][1] if not r.empty else 0.5
        scores.append(p)
    ...
    team_tag_texts = team_rows[[c for c in champ_cols]].fillna('').astype(str).agg(','.join, axis=1)
